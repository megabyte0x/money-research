...
test('every direct destination and numeric alias has one canonical, unique article identity', () => {
  ...
  for (const record of manifest) {
    ...
    for (const alias of record.aliases) {
      const aliasHtml = page(record, alias);
      assert.equal(aliasHtml, html, `${record.id}: alias ${alias} must render the same chapter and canonical`);
    }
  }
  assert.equal(titles.size, 44);
  assert.equal(canonicals.size, 44);
});

test('crawlable section links land on their exact chapter section', () => {
  const canonical = new Map(manifest.map(record => [`/${record.vol}/${record.slug}/`, record]));
  const alias = new Map(manifest.flatMap(record => record.aliases.map(slug =>
    [`/${record.vol}/${slug}/`, `/${record.vol}/${record.slug}/`])));
  let sectionLinks = 0;
  for (const record of manifest) {
    const html = page(record);
